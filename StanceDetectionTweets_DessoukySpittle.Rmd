...
# Import manually reviewed workbook ######################################################
...
## Subsequent SemEval Task 6 Progress  
...
Additionally, the authors of the original dataset made the choice to split the train and test data chronologically, which may have adversely effected the comparability of the train and test set. This may be a borader limitation of Tweet analysis in general: given the velocity of conversation it may be difficult to train and maintain a model that stays relevant. But the acute issue in this dataset is if something happened between the train and test data where people began using a new hashtag or discussing something in the news, our model will not have been trained to detect that context.  

In addition, there was a high level of disagreement amongst the label 'encoders', which translates to a higher expected error rate in our model. We can expect our model to struggle classifying the stance of tweets in which humans do not consistently agree. Further, the total number of tweets in our dataset was low for what is standard for a NLP problem. This may affect the reproducibility of our model results.  Re-running the same model on the identical training and test sets would produce slightly different results, despite fitting the training data to near 100% accuracy.
...
Errors for target **Atheism** were manually reviewed to better understand the nature of mis-classified tweets. Errors were categorized according to whether the purported true label or the predicted label from our model appeared to be correct (or if neither appeared correct). _\autoref{fig:error_analysis}_ in the *Appendix* shows the results of this error review and annotation.   
...
Similarly, _Referential_ errors occur when other tweets reference a target other than the target of interest and an unreasonable leap in assumption is made to come to a conclusion on that author's potential stance on the at-issue target. This is apparent in the below tweet where the initial query hashtag, which is not presented (replaced with _#SemST_), could be the point that makes the author's stance clear. However, without it, it is not reasonable to draw the conclusion that celebrating Independence Day and the constitution exhibits a stance against Atheism.  
